...
- Basic заголовок (единое поле) вместо логина/пароля, автоматическая миграция со старого формата.
- Список микросервисов с индивидуальным включением (checkbox) и авто-обнаружением имени сервиса (пинг корневого URL).
- Автообновление токена через `chrome.alarms` с заданным интервалом (по умолчанию 9 минут).
...
Корневые устаревшие файлы (`background.js`, `contentScript.js`, `options.html/js`, `popup.html/js`) удалены после переноса в `src/`.

## Установка (Dev)
1. Chrome → Extensions → включить Developer Mode.
2. Нажать `Load unpacked` и выбрать корень `auto-swagger-token`.
...
## Идеи для улучшений
- Шифрование Basic header.
- Разные схемы авторизации для разных сервисов.
- Индикатор времени до истечения (если endpoint возвращает expire).
- Экспорт / импорт настроек.

## Лицензия
Внутреннее использование. При необходимости добавьте файл LICENSE.
...
2. Ввести `Basic Header` полностью (например `Basic YWRtaW46c1RyMG4mZ2c=`) или только base64 часть.
